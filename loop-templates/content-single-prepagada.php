		...
		<article class="box-mascotas-list mr-md-4 mb-5" <?php post_class(); ?> id="post-<?php the_ID(); ?>">
			...
			<div class="entry-content mb-5">
					...
					<?php if( have_rows('lista_de_planes') ): ?>
						...
						<?php while( have_rows('lista_de_planes') ): the_row();?>
							<div class="card-header" id="heading-<?php the_sub_field('no_item'); ?>">
								<h2 class="mb-0" data-toggle="collapse" data-target="#collapse-<?php the_sub_field('no_item'); ?>" aria-expanded="<?php if ( the_sub_field('open') == true ) { ?>false<?php } else { ?>true<?php } ?>" aria-controls="collapse-<?php the_sub_field('no_item'); ?>">
								<?php the_sub_field('titulo_plan'); ?>
								</h2>
							</div>
							<div id="collapse-<?php the_sub_field('no_item'); ?>" class="collapse <?php if ( the_sub_field('open') == true ) { ?>e<?php } else { ?>show<?php } ?>" aria-labelledby="heading-<?php the_sub_field('no_item'); ?>" data-parent="#accordionExample">
				...
				<div class="w-100 mt-5 mb-5">
					<div class="medio-marketing mb-lg-5 pb-lg-5">
